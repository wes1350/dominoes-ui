...
class Board extends React.Component {
    ...
    componentDidMount() {
        ...
        this.props.socket.on("CLEAR_BOARD", () => {
            this.setState({
                domino_faces: [],
                domino_locations: [],
    ...
    calculate_domino_size(state) {
        ...
        let img_size =
            state.span *
            Math.round(state.n_pixels / Math.max(board_width, board_height));
        return img_size;
    }
    ...
    check_board_size(coords1, coords2) {
        ...
        if (
            coords1[0] + east_offset < west ||
            coords2[0] + east_offset < west
        ) {
            if (east - east_edge >= 8) {
            ...
            coords1[0] + east_offset + 2 > east ||
            coords2[0] + east_offset + 2 > east
        ) {
            if (west - west_edge <= -8) {
            ...
            coords1[1] + north_offset > north ||
            coords2[1] + north_offset > north
        ) {
            ...
            coords1[1] + north_offset - 2 < south ||
            coords2[1] + north_offset - 2 < south
        ) {
    ...
    render() {
        ...
        for (i = 0; i < this.state.domino_faces.length; i++) {
            domino_info.push({
                faces: this.state.domino_faces[i],
        ...
        return (
            <div
                ...
                style={{
                    ...this.props.style,
                    gridTemplateColumns:
                        "repeat(" + max_dimension + ", " + square_size + "px)",
                    gridTemplateRows:
                        "repeat(" + max_dimension + ", " + square_size + "px)"
                }}
            >
